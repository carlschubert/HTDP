...
(define (chart los)
  (cond [(empty? los) (square 0 "solid" "white")]
        [else
         (beside/align "bottom" (render_bar (first los))
                       (chart (rest los)))]))

;; school -> Image
;; takes a school and returns an image of a bar
#;
(define (render_bar s) (square 0 "solid" "white")) 

(check-expect (render_bar (make-school "S1" 25000)) (overlay/align "center" "bottom"
                                     (rotate 90 (text "S1" FONT-SIZE FONT-COLOR))
                                     (rectangle BAR-WIDTH (* 25000 Y-SCALE) "outline" "black")
                                     (rectangle BAR-WIDTH (* 25000 Y-SCALE) "solid" BAR-COLOR)))
...
;(check-expect (chart LOS2) (square 0 "solid" "white"))


(chart (cons S2 (cons S1 empty)))
